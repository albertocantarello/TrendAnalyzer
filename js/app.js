...
async function fetchData() {
    ...
    var loadingRing = document.getElementById('loading');
    var updateTime = document.getElementById('update-time');

    try {
        var response = await fetch('data/trends.json?t=' + new Date().getTime());
        if (!response.ok) {
        ...
        if(data.last_update && data.last_update.includes("dimostrativo")) {
             updateTime.innerHTML = '<span class="text-yellow-400"><i class="fas fa-exclamation-circle mr-1"></i> Dati Demo</span>';
        } else {
        ...
        container.innerHTML =
            '<div class="col-span-1 md:col-span-2 lg:col-span-3 flex flex-col items-center justify-center text-center p-12 glass-card rounded-2xl border-yellow-500/20">' +
                ...
                '<p class="text-slate-400 max-w-md">I trend settimanali non sono ancora stati generati.</p>' +
            '</div>';
    ...
    trends.forEach(function(trend, index) {
        ...
        card.innerHTML =
            '<div class="flex justify-between items-start mb-5 gap-3">' +
                ...
                '<span class="px-2.5 py-1 rounded-md text-xs font-semibold border flex items-center shrink-0 ' + freqColor + '">' +
